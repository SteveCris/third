...
public class OrderRepaymentController {
	...
	@ApiOperation(value = "试算接口")
	@ResponseBody
	@RequestMapping(value = "/pilotcalculation", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public CoreResult<OrderRepaymentPlanVo> pilotcalculation(@PathVariable("appid") String appid,
			@RequestParam("orderId") String orderId) {
		AssertUtils.notNullOrBlank(appid, "appid 不能为空");
		AssertUtils.notNullOrBlank(orderId, "orderId 不能为空");

		return CoreResultEnum.SUCCESS.with(orderRepaymentService.pilotcalculation(appid, orderId));
	}
