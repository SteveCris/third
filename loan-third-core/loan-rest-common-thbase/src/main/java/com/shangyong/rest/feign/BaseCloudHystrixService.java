...
public interface BaseCloudHystrixService {
	...
	RestResult<Void> checkBank(@PathVariable("appid") String appid, @RequestParam("cardType") int cardType,
	...
	public static class HystrixClientFallbackFactory implements FallbackFactory<BaseCloudHystrixService> {
		...
		public BaseCloudHystrixService create(Throwable cause) {
			return new BaseCloudHystrixService() {
				...
				@Override
				public RestResult<Void> checkBank(String appid, int cardType, String bankCode) {
					logger.error("error cause baseCloudHystrixService checkBank appid:{} cardType:{} bankCode:{}",
							appid, cardType, bankCode, cause);
					return null;
				}
